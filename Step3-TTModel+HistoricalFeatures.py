...
# ── Idea 1: Historical workflow features ──────────────────────────────────
# Sort by time if available, otherwise use existing order
time_col = next((c for c in df.columns if "created" in c.lower() or "started" in c.lower()), None)
if time_col:
    df = df.sort_values(time_col).reset_index(drop=True)
    print(f"  Sorted by: {time_col}")
else:
    print("  No timestamp column found — using existing row order for history")

grp = df.groupby("repo_workflow_id")[TARGET_COL]

# Rolling stats over last 5 runs (shift(1) prevents data leakage)
...
df["hist_median_5"] = grp.transform(lambda x: x.shift(1).rolling(5, min_periods=1).median())
df["hist_min_5"]    = grp.transform(lambda x: x.shift(1).rolling(5, min_periods=1).min())
df["hist_max_5"]    = grp.transform(lambda x: x.shift(1).rolling(5, min_periods=1).max())

# Last single run duration
df["hist_last_1"]   = grp.transform(lambda x: x.shift(1))

# Fill NaNs for first runs of each workflow with global mean
global_dur_mean = df[TARGET_COL].mean()
for col in ["hist_mean_5", "hist_std_5", "hist_median_5",
            "hist_min_5", "hist_max_5", "hist_last_1"]:
    df[col] = df[col].fillna(global_dur_mean)
